...
class questionController extends Controller
{
    ...
    public function index(Request $request)
    {
        $questions = question::where('status_view', 1)->when($request->searsh, function ($query) use ($request) {
            return $query->where('text_question', 'like', '%' . $request->searsh . '%')->orwhere('text_question_en', 'like', '%' . $request->searsh . '%');
        })->paginate(5);
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'text_question_en' => 'required|unique:questions|max:255',
            'text_answer_en' => 'required|max:255',
            'center_type' => 'required|numeric'
        ]);
        $request_data = $request->only(['text_question', 'text_answer', 'center_type','text_question_en','text_answer_en']);
    ...
    public function update(Request $request, question $question)
    {
        $request->validate([
            ...
            'text_question_en' => 'required|string|max:255|' . Rule::unique('questions')->ignore($question->text_question_en, 'text_question_en'),
            'text_answer_en' => 'required|string|max:255',
            'center_type' => 'required|numeric',
            'status_view' => 'required|numeric'

        ]);
        $request_data = $request->only(['text_question', 'text_answer','text_question_en', 'text_answer_en', 'center_type', 'status_view']);
