...
class welcomeController extends Controller
{
    ...
    public function admin(Request $request)
    {
        ...
        $count_telecome = file_get_contents('http://localhost/billingCorporation/public/api/archivedTelecomeBill/count?searsh=' . $request->searsh);
        $count_telecome = json_decode($count_telecome, true);
        $count_telecome = $count_telecome['count'];

        $count_water = file_get_contents('http://localhost/billingCorporation/public/api/archivedWaterBill/count?searsh=' . $request->searsh);
        $count_water = json_decode($count_water, true);
        $count_water = $count_water['count'];

        $count_electricty = file_get_contents('http://localhost/billingCorporation/public/api/archivedElectricityBill/count?searsh=' . $request->searsh);
