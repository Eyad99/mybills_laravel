...
class billTelecomeController extends Controller
{
    public function pay($phone_number, $course_number, $id_bank, Request $request)
    {
        ...
        $bill = file_get_contents("http://localhost:80/billingCorporation/public/api/getNewTelecomeBill/$phone_number/$course_number");
        $bill = json_decode($bill);
        $bill = $bill->data;
        //dd($bill);
        $amount_due_of_payment = $bill->amount_due_of_payment;
        $result_check = file_get_contents('http://localhost:80/bemoBank/public/api/checkInformation/' . $id_bank . '/value/' . $amount_due_of_payment . '?number=' . $phone_number . '&course_number=' . $course_number . '&' . 'bill_type=telecome');

        $result_check = json_decode($result_check);
        
        if ($result_check[0] == true) {
            file_get_contents('http://localhost:80/billingCorporation/public/api/complatePayTelecome/' . $phone_number . '/' . $course_number . '/' . $result_check[1] . '/' . auth()->id());
            ...
            if(app()->getLocale()=='ar')
            return \redirect('http://127.0.0.1:8000/ar/myBills/new/telecome?phone_number=' . $phone_number);
            else
    ...
    {
        $bill = file_get_contents('http://localhost:80/billingCorporation/public/api/getArchivedTelecomeBill/' . $phone_number . '/' . $course_number);
        $bill = json_decode($bill);
        $bill = $bill->data;
    ...
    public  function archived(Request $request)
    {
        ...
        $telecome_statistic = file_get_contents('http://localhost:80/billingCorporation/public/api/telecome/RestoreConsumptionRates?phone_number=' . $request->phone_number);
        $telecome_statistic = json_decode($telecome_statistic);
        $statistic = $telecome_statistic->data;
        $code = $telecome_statistic->code;
        $phone_number = $request->phone_number;
        $data = file_get_contents('http://localhost:80/billingCorporation/public/api/archivedTelecomeBill?searsh=' . $phone_number);
