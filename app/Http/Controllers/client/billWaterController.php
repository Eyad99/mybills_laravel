...
class billWaterController extends Controller
{
    ...
    {
        $bill = file_get_contents("http://localhost:80/billingCorporation/public/api/getNewWaterBill/$counter_number/$course_number");
        $bill = json_decode($bill);
        $bill = $bill->data;
        
        $amount_due_of_payment = $bill->amount_due_of_payment;
        $result_check = file_get_contents('http://localhost:80/bemoBank/public/api/checkInformation/' . $id_bank . '/value/' . $amount_due_of_payment . '?number=' . $counter_number . '&course_number=' . $course_number . '&' . 'bill_type=water');

        $result_check = json_decode($result_check);
        
        if ($result_check[0] == true) {
            file_get_contents('http://localhost:80/billingCorporation/public/api/complatePayWater/' . $counter_number . '/' . $course_number . '/' . $result_check[1] . '/' . auth()->id());
            ...
            if ($send_email) {
                ...
                if(app()->getLocale()=='ar')
                return \redirect('http://127.0.0.1:8000/ar/myBills/new/water?counter_number=' . $counter_number);
                else
    ...
    {
        $bill = file_get_contents('http://localhost:80/billingCorporation/public/api/getArchivedWaterBill/' . $counter_number . '/' . $course_number);
        $bill = json_decode($bill);
        $bill = $bill->data;
        // dd($bill);
    ...
    public  function archived(Request $request)
    {
        ...
        $water_statistic = file_get_contents('http://localhost:80/billingCorporation/public/api/water/RestoreConsumptionRates?counter_number=' . $request->counter_number);
        ...
        $data = file_get_contents('http://localhost:80/billingCorporation/public/api/archivedWaterBill?searsh=' . $counter_number);
