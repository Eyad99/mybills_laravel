...
class billElecticityController extends Controller
{
    public function pay($hour_number, $course_number, $id_bank)
    {
        ...
        $bill = file_get_contents("http://localhost:80/billingCorporation/public/api/getNewElectricityBill/$hour_number/$course_number");
        $bill = json_decode($bill);
        $bill = $bill->data;

        $amount_due_of_payment = $bill->amount_due_of_payment;
        $result_check = file_get_contents('http://localhost:80/bemoBank/public/api/checkInformation/' . $id_bank . '/value/' . $amount_due_of_payment . '?number=' . $hour_number . '&course_number=' . $course_number . '&' . 'bill_type=electricity');

        $result_check = json_decode($result_check);
        if ($result_check[0] == true) {
            file_get_contents('http://localhost:80/billingCorporation/public/api/complatePayElectricity/' . $hour_number . '/' . $course_number . '/' . $result_check[1] . '/' . auth()->id());
            ...
            if(app()->getLocale()=='ar')
            return \redirect('http://127.0.0.1:8000/ar/myBills/new/electricity?hour_number=' . $hour_number);
    ...
    {
        $bill = file_get_contents('http://localhost:80/billingCorporation/public/api/getArchivedElectricityBill/' . $hour_number . '/' . $course_number);
        $bill = json_decode($bill);
        $bill = $bill->data;
        $receipt = file_get_contents('http://localhost:80/bemoBank/public/api/getReceipt/' . $bill->receipt_id);
    ...
    public  function archived(Request $request)
    {
        ...
        $electricity_statistic = file_get_contents('http://localhost:80/billingCorporation/public/api/electricity/RestoreConsumptionRates?hour_number=' . $request->hour_number);
        ...
        $data = file_get_contents('http://localhost:80/billingCorporation/public/api/archivedElectricityBill?searsh=' . $hour_number);
